GLET.controller('ScheduleKEditTaxReturnFormController', function ($rootScope, $filter, $compile, localStorageService, $route, $scope, WebAPIService, authService, $timeout, constant, dataService, $http, appSetting, $location) {
    ...
    $scope.pdfshow1 = false;
    $scope.active3 = true;
    $scope.isAlwaysDisabled = true;
    ...
    $scope.SelectedYearID = localStorage.getItem('YearId');

   
    $scope.PrintElem = function (elem) {
        var link = window.document.createElement("a");
        link.download = "Form_1120_IC-DISC_ScheduleK.pdf";
        //  alert($scope.selectedName)
        if (typeof $scope.selectedName == 'undefined' || $scope.selectedName == null) {
            link.href = appSetting.ServerPath + "apiGet/TaxReturnCompliance/GetScheduleKPDFService?EntityID=" + localStorage.getItem('EntityId') + "&YearID=" + localStorage.getItem('YearId') + "&CaseID=" + localStorage.getItem('caseID') + "&OwnerName=";
        }
        else {
            link.href = appSetting.ServerPath + "apiGet/TaxReturnCompliance/GetScheduleKPDFService?EntityID=" + localStorage.getItem('EntityId') + "&YearID=" + localStorage.getItem('YearId') + "&CaseID=" + localStorage.getItem('caseID') + "&OwnerName=" + $scope.selectedName + "";
        }
        link.click();
    }
    var RowIDNameArray = [];
    ...
    $scope.getDataFromExcel = function () {
        ...
        $scope.input1a = $scope.ctx.excelIOSheet.getCellValue(8, 6, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input1b = $scope.ctx.excelIOSheet.getCellValue(8, 7, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input1c = $scope.ctx.excelIOSheet.getCellValue(8, 8, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input2 = $scope.ctx.excelIOSheet.getCellValue(8, 9, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input3 = $scope.ctx.excelIOSheet.getCellValue(8, 10, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input4a = $scope.ctx.excelIOSheet.getCellValue(8, 11, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input4b = $scope.ctx.excelIOSheet.getCellValue(8, 12, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input5 = $scope.ctx.excelIOSheet.getCellValue(8, 13, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input6 = $scope.ctx.excelIOSheet.getCellValue(8, 14, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input7 = $scope.ctx.excelIOSheet.getCellValue(8, 15, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input8 = $scope.ctx.excelIOSheet.getCellValue(8, 16, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input9 = $scope.ctx.excelIOSheet.getCellValue(8, 17, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.input10 = $scope.ctx.excelIOSheet.getCellValue(8, 18, false, $scope.ctx.excelIOSheet.sheets[0]);

        $scope.address1 = $scope.ctx.excelIOSheet.getCellValue(8, 0, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.address2 = $scope.ctx.excelIOSheet.getCellValue(8, 1, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.address3 = $scope.ctx.excelIOSheet.getCellValue(8, 2, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.address4 = $scope.ctx.excelIOSheet.getCellValue(8, 3, false, $scope.ctx.excelIOSheet.sheets[0]);
        $scope.address5 = $scope.ctx.excelIOSheet.getCellValue(8, 4, false, $scope.ctx.excelIOSheet.sheets[0]);
    }
    $scope.loaderDivShowHide = true;
    ...
    $scope.initializeExcelIOSheet = function (flexSheet) {
        ...
        $("#loader").show();
        $http.get(appSetting.clientServerPath + 'Test/schk.xlsx', { responseType: 'arraybuffer' })
        ...
        //wijmo_display_id hiding
        $('#wijmo_display_id').css('visibility', 'hidden');
    };
